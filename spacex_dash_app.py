...
def update_pie_chart(selected_site):
    if selected_site == 'ALL':
        ...
        # Calculate successful vs. failed launches for the selected site
        site_df = spacex_df[spacex_df['Launch Site'] == selected_site]
        total_success = site_df[site_df['class'] == 1]['class'].count()
        total_failed = site_df[site_df['class'] == 0]['class'].count()
        labels = ['Successful', 'Failed']
        values = [total_success, total_failed]

    fig = px.pie(names=labels, values=values, title=f"Success Distribution for {selected_site}")
    return fig
...
def update_scatter_chart(selected_site, payload_range):
    ...
    fig = px.scatter(filtered_df, x='Payload Mass (kg)', y='class',
                     color='class', title=f'Payload vs. Success for {selected_site}')
    return fig
